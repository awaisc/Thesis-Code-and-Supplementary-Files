...
# gtfUCSCexonscoding<-import("~/DataFiles/Gene Tracks/Mouse/FullMm9genome.GTF")


##Human inputs

enhancerGrangeHumans<-import(con="~/DataFiles/Enhancer Tracks/Human/human_permissive_enhancers_phase_1_and_2.bed")
UCSCgenesHuman<-import(con= "~/DataFiles/Gene Tracks/Human/hg.bed")
promotersHuman<-promoters(UCSCgenesHuman)
gtfUCSChHumans<-import(con="~/DataFiles/Gene Tracks/Human/hg19.gtf")

##Inputs
# enhancerGrange<-enhancerGrangeHumans
# UCSCgenes<-UCSCgenesHuman
# promoters<-promotersHuman
# gtfUCSCexonscoding<-gtfUCSChHumans

genomeInput<-BSgenome.Hsapiens.UCSC.hg19


arx6MerPWMNospace<-cbind(round(PWM("TAATT")*7),
                         round(PWM("TTAAT")*7))

arx6MerPWM1space<-cbind(round(PWM("TAATT")*7),
                        0.25,
                        round(PWM("TTAAT")*7))

arx6MerPWM2space<-cbind(round(PWM("TAATT")*7),
                         0.25,0.25,
                         round(PWM("TTAAT")*7))
arx6MerPWM3space<-cbind(round(PWM("TAATT")*7),
                        0.25,0.25,0.25,
                        round(PWM("TTAAT")*7))
arx6MerPWM4space<-cbind(round(PWM("TAATT")*7),
                        0.25,0.25,0.25,0.25,
                        round(PWM("TTAAT")*7))
arx6MerPWM5space<-cbind(round(PWM("TAATT")*7),
                        0.25,0.25,0.25,0.25,0.25,
                        round(PWM("TTAAT")*7))
arx6MerPWM6space<-cbind(round(PWM("TAATT")*7),
                        0.25,0.25,0.25,0.25,0.25,0.25,
                        round(PWM("TTAAT")*7))
arx6MerPWM7space<-cbind(round(PWM("TAATT")*7),
...
arxTandemNoSpace<-cbind(round(PWM("TAATT")*7),
                           round(PWM("AATTA")*7))
arxTandem1Space<-cbind(round(PWM("TAATT")*7),
                          0.25,
                          round(PWM("AATTA")*7))
arxTandem2Space<-cbind(round(PWM("TAATT")*7),
                          0.25,0.25,0.25,
                          round(PWM("AATTA")*7))
arxTandem3Space<-cbind(round(PWM("TAATT")*7),
                          0.25,0.25,0.25,
                          round(PWM("AATTA")*7))
arxTandem4Space<-cbind(round(PWM("TAATT")*7),
                          0.25,0.25,0.25,0.25,
                          round(PWM("AATTA")*7))
arxTandem5Space<-cbind(round(PWM("TAATT")*7),
                          0.25,0.25,0.25,0.25,0.25,
                          round(PWM("AATTA")*7))
arxTandem6Space<-cbind(round(PWM("TAATT")*7),
                          0.25,0.25,0.25,0.25,0.25,0.25,
                          round(PWM("AATTA")*7))
arxTandem7Space<-cbind(round(PWM("TAATT")*7),
                          0.25,0.25,0.25,0.25,0.25,0.25,0.25,
                          round(PWM("AATTA")*7))
...
grangeplaindromicNospace <-
  matchPWM(arx6MerPWMNospace, genomeInput, "100%")
grangeplaindromic1space <-
  matchPWM(arx6MerPWM1space, genomeInput, "100%")
grangeplaindromic2space <-
  matchPWM(arx6MerPWM2space, genomeInput, "100%")
grangeplaindromic3space <-
  matchPWM(arx6MerPWM3space, genomeInput, "100%")
grangeplaindromic4space <-
  matchPWM(arx6MerPWM4space, genomeInput, "100%")
grangeplaindromic5space <-
  matchPWM(arx6MerPWM5space, genomeInput, "100%")
grangeplaindromic6space <-
  matchPWM(arx6MerPWM6space, genomeInput, "100%")
grangeplaindromic7space <-
  matchPWM(arx6MerPWM7space, genomeInput, "100%")

grangeTandemNoSpace<-
  matchPWM(arxTandemNoSpace, genomeInput, "100%")
grangeTandem1space <-
  matchPWM(arxTandem1Space, genomeInput, "100%")
grangeTandem2space <-
  matchPWM(arxTandem2Space, genomeInput, "100%")
grangeTandem3space <-
  matchPWM(arxTandem3Space, genomeInput, "100%")
grangeTandem4space <-
  matchPWM(arxTandem4Space, genomeInput, "100%")
grangeTandem5space <-
  matchPWM(arxTandem5Space, genomeInput, "100%")
grangeTandem6space <-
  matchPWM(arxTandem6Space, genomeInput, "100%")
grangeTandem7space <-
  matchPWM(arxTandem7Space, genomeInput, "100%")
...
rownames(tandemDataTable) <- c("No Space",
                               "ARX Jolma",
                               "2 Space",
                               "3 Space",
                               "4 Space",
...
write.table(planindromicDataTable, file = "~/Thesis/OrientationHumanJolmaTandem",
            sep= "\t",
            append= FALSE,
            row.names = FALSE,
            quote=FALSE)
...
colnames(planindromicDataTable) <- c("Total",
                                     ...
                                     "Motifs in Enhancers",
                                     "Non Coding")
rownames(planindromicDataTable) <-c(
                                    "No Space",
                                    "1 Space",
                                    "2 Space",
                                    "3 Space",
...
write.table(planindromicDataTable, file = "~/Thesis/OrientationHumanJolmaPalindromic",
            sep= "\t",
            append= FALSE,
            row.names = FALSE,
            quote=FALSE)
...
ggplot(ggplotdataFrameDistanceofTandemicMotifsFromPromoter, aes(x=Distance, group=Space, color=Space))+
  geom_density(bins = 500)+
  theme_bw()+
  xlab(label = "Distance To The Closest Transcription Start Site(Base Pairs)")+
  ylab(label= "Proportion of Motifs")+
  theme(text = element_text(size=12))+
  scale_x_continuous(limits = c(0, 200000))
...
ggplot(ggplotdataFrameDistanceofPlaindromicMotifsFromPromoter, aes(x=Distance, group=Space))+
  geom_density(bins = 500, aes(colour=Space))+
  theme_bw()+
  theme(text = element_text(size=16))+
  xlab(label = "Distance To Closest Transcription Start Site(Base Pairs)")+
  ylab(label= "Number Of Motifs")+
